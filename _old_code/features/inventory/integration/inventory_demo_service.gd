...
func delete_item(id_: String) -> bool:
	if _inventory.has(id_):
		var __ := _inventory.erase(id_)
		return true
	if _ground.has(id_):
		var __ := _ground.erase(id_)
		return true
	return false
...
func seed_demo_data() -> void:
	var __ := create_item(true, "Rusty Sword", "A worn 1h sword.", Category.WEAPON, false, 1)
	__ = create_item(true, "Cloth Hood", "Basic cloth headgear.", Category.EQUIPMENT, false, 1)
	__ = create_item(true, "Health Potion", "Restores HP.", Category.CONSUMABLE, true, 5)
	__ = create_item(false, "Iron Ore", "Harvested ore.", Category.MATERIAL, true, 12)
	__ = create_item(false, "Ancient Letter", "Quest evidence.", Category.QUEST_ITEM, false, 1)
