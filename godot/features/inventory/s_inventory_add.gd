...
func process(entities: Array[Entity], components: Array, delta: float):
	for inv in entities:
		...
		if req.item_entity_id == -1 or req.count <= 0:
			cmd.remove_component(inv, C_InventoryAddRequest)
			continue
		...
		if stack != null and stack.is_stackable:
			var to_add := min(req.count, stack.count)
		...
		if not InventoryRelation.can_add(inv, 1):
			cmd.remove_component(inv, C_InventoryAddRequest)
			continue

		cmd.add_relationship(inv, InventoryRelation.contains_item(item, 1))
		cmd.remove_component(inv, C_InventoryAddRequest)
...
func _try_merge_stack(inv: Entity, incoming_item: Entity, add_count: int) -> bool:
	...
	for rel in rels:
		var existing_item := rel.target as Entity
		...
		var actual := min(space, add_count)
