	...
	set(value):
		...
		if server_world:
			if not server_world.is_inside_tree():
				...
				get_tree().root.get_node("./Root").add_child(server_world)
			if not server_world.tree_exited.is_connected(ECS._on_world_exited):
				UIUtils.safe_connect(server_world.tree_exited, ECS._on_world_exited, "GameOrchestrator _on_world_exited")
		world_changed.emit(server_world)
	...
	set(value):
		...
		if client_world:
			if not client_world.is_inside_tree():
				# Add the world to the tree if it is not already
				get_tree().root.get_node("./Root").add_child(client_world)
			if not client_world.tree_exited.is_connected(ECS._on_world_exited):
				UIUtils.safe_connect(client_world.tree_exited, ECS._on_world_exited, "GameOrchestrator _on_world_exited")
		world_changed.emit(client_world)
