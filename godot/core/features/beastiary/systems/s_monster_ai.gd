...
func process(entities: Array[Entity], _components: Array, _delta: float) -> void:
	...
	for entity in entities:
		var monster_pos := (entity.get_component(C_Transform) as C_Transform).transform.origin
		...
		for player: Entity in players:
			var player_pos := (player.get_component(C_Transform) as C_Transform).transform.origin
