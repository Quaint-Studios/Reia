...
func process(entities: Array[Entity], _components: Array, _delta: float):
	...
	for entity in entities:
		...
		var is_near = bank_npcs.any(func(npc):
			return player_pos.distance_to(npc.get_component(C_Transform).transform.origin) <= 5.0
		)
		
		if not is_near:
			cmd.add_component(entity, C_ActionBlocked.new(InventoryErrors.TOO_FAR_FROM_BANK))
