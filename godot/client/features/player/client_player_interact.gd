...
@onready var camera: Camera3D = self as Camera3D
...
func _physics_process(_delta: float) -> void:
	...
	var end = origin + camera.project_ray_normal(center) * INTERACT_RANGE
	...
	if result and result.collider:
		var hit_node = result.collider.get_parent() # The StaticBody's parent is the Entity
